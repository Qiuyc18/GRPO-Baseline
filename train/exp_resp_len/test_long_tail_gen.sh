...
# DeepScaleR GRPO run for reproducing rollout long-tail timing on MI200.
# This is intentionally fresh-rollout only: no replay buffer and no spec verify.
# The defaults raise response length and use NAT token sampling to shorten actor
# updates, so rollout generation should dominate the measured phase time.
...
export WANDB_RUN_GROUP="${WANDB_RUN_GROUP:-deepscaler_4gpu_long_tail_gen}"
export TRAIN_LOGGER="${TRAIN_LOGGER:-[\"console\",\"wandb\"]}"

# ============ Training knobs ============
# Baseline 2k response runs had rollout/update_actor near 50/50 on phase time.
# Raising the response cap exposes the long tail; NAT reduces actor update work
# without changing fresh rollout generation or verifier reward.
...
# NAT knobs. For targeting gen at 70-80% phase time, the most useful sweep is:
# MAX_RESPONSE_LENGTH=4096/6144/8192 and NAT_KEEP_RATIO=0.20/0.25/0.35.
export NAT_TOKEN_SAMPLING="${NAT_TOKEN_SAMPLING:-True}"
export NAT_MODE="${NAT_MODE:-rpc_urs}"
export NAT_KEEP_RATIO="${NAT_KEEP_RATIO:-0.25}"
export NAT_MIN_TOKENS="${NAT_MIN_TOKENS:-1}"
export NAT_TRUNCATE_RATIO="${NAT_TRUNCATE_RATIO:-0.25}"
export NAT_TRUNCATE_MIN_TOKENS="${NAT_TRUNCATE_MIN_TOKENS:-512}"
...
util = float(os.environ.get("GPU_MEMORY_UTILIZATION", "0.78"))
try:
...
echo ">>> Start DeepScaleR long-tail timing run, fresh rollout + NAT token sampling"
echo "    Log file: ${LOG_FILE}"
echo "    Stop: kill \$(cat ${PID_FILE})"
echo "    Response cap: ${MAX_RESPONSE_LENGTH}, rollout.n=${ROLLOUT_N}, train_batch=${TRAIN_BATCH_SIZE}"
...
echo "    After manual run: python3 scripts/analyze_training_time_by_step.py ${GPU_MONITOR_OUTPUT} --no-plot"
