...
# Small baseline smoke test for Qwen3-4B-Base on DeepScaleR with replay disabled.
# Keep this script aligned with test_qwen3_4B-base_deepscaler_replay_buffer_smoke.sh
# so the timing comparison is mostly about replay vs fresh rollout.
...
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-test_qwen3_4B-base_deepscaler_no_replay_smoke}"
...
# Training knobs chosen to match the replay-buffer smoke test.
...
mkdir -p "${CKPT_ROOT}" "${SMOKE_DATA_DIR}"
...
def take_rows(src: Path, n: int) -> pa.Table:
    ...
    for batch in pf.iter_batches(batch_size=1024):
        ...
        if kept:
            selection = kept[: max(0, n - count)]
            chunks.append(pa.Table.from_pylist(selection))
            count += len(selection)
        if count >= n:
            break
    if count < n:
...
echo ">>> Start Qwen3-4B-Base DeepScaleR no-replay smoke test"
