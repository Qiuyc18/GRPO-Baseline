...
# Full DeepScaleR GRPO run on Qwen3-4B-Base, 4 GPUs, no replay buffer.
# Intended as the fresh-rollout baseline for replay-buffer comparison.
...
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-train_qwen3_4B-base_deepscaler_grpo_4gpu_no_replay}"
...
export WANDB_RUN_GROUP="${WANDB_RUN_GROUP:-deepscaler_4gpu_replay_compare}"
export TRAIN_LOGGER="${TRAIN_LOGGER:-[\"console\",\"wandb\"]}"

# ============ Training knobs ============
# 4 GPUs with tensor_model_parallel_size=2 gives 2 data-parallel ranks.
# Defaults keep about 64 prompts per DP rank and 5 rollouts per prompt.
...
echo ">>> Start full DeepScaleR GRPO training, no replay"
