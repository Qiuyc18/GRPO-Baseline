...
export GPUS_PER_NODE="${GPUS_PER_NODE:-6}"  # 每个节点 GPU 数量，显卡不足时改小
export EXPERIMENT_NAME="train_qwen3_4B-base_deepscaler_grpo_6gpu"
...
# ============ 训练参数说明 ============
# Qwen3-4B-Base, 6x AMD GPU, tensor_parallel=2 → data_parallel=3
#
# [6 卡适配思路]:
#   维持每个 data-parallel rank 的样本负载接近原配置:
#     train_batch_size: 256(4*64) -> 192(3*64)
#   维持与原配置相近的梯度累积节奏:
#     ppo_mini_batch_size: 64 -> 48 (= 3 * micro_batch(2) * 8)
#   其余显存相关参数保持不变:
